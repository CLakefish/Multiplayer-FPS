...
public void AllowSpawnServerRPC(bool value)
{
    ...
    void Update()
    {
        if (IsOwner) {
            ...
            transform.position = Vector3.SmoothDamp(transform.position, state.Value.Position, ref currentVelocity, 0.025f);
            transform.rotation = Quaternion.Euler(new Vector3(0, Mathf.SmoothDampAngle(transform.rotation.eulerAngles.y, state.Value.Rotation.y, ref currentRotation, 0.025f), 0));
        }
    }
}
...
public abstract class NetworkTickBehaviour : NetworkBehaviour {
    ...
    public float timer, minTimeBetweenTicks;
